...
# @app.put("/items/create/", response_model=Item, tags=["items"])
...
#     return item
# @app.post("/items/with-params", response_model=Item, tags=["items"])
# async def create_item_with_params(
#     id: int = Query(...),
#     name: str = Query(...),
#     price: float = Query(...),
...
# ):
#     """
#     Create a new item in the database using parameters.
...
async def create_item_tool(name: str, price: float, description: Optional[str] = None, tags: List[str] = []) -> dict:
    ...
    return new_item.model_dump()  # Return the created item as a dictionary
